...
Int_t StMCAsymMaker::Make() {
    ...
    if ((flavor1*flavor2)<0){
      if ((flavor3*flavor4)<0) {
	if (abs(flavor1)==abs(flavor2)){
	  if (abs(flavor3)==abs(flavor4)){
	    cout << "****Subprocess pid="<<pid<<" flavor1: " << flavor1 << "  flavor2: " << flavor2 <<"  flavor3: "<<flavor3<<"  flavor4: "<<flavor4<<endl;
	  }
	} 
      }
    }
...
Double_t StMCAsymMaker::getPartonicALL(double s, double t, double u, int sub, int inA, int inB, int outA, int outB){
    ...
    if ((sub==11)&&(inA==(-1*inB))&&(outA==(-1*outB))&&(inA==outA)&&(inB==outB)) {
      all=N4/D4;
      cout<<"   This all was ="<<N2/D2<<" and is now ="<<N4/D4<<endl;
    }
